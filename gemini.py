...
if graph_mode == "Separate Subplots":
    ...
    if graph_mode == "Both":
        # Make the PWM input slightly more transparent to avoid visual domination
        ax.plot(t, pwm, linestyle="--", linewidth=1.0, alpha=0.4, color="#1f77b4", label="PWM Input (V)")
...
if advanced_feature == "FFT Analyzer":
    ...
    st.subheader("🖥 True Oscilloscope Display (Auto-Zoom & Dual Axis)")
    fig, ax = plt.subplots(figsize=(12, 5), facecolor="black")
    ax.set_facecolor("black")
    
    # Auto-zoom to show exactly 5 cycles of the PWM wave (like a real oscilloscope)
    cycles_to_show = 5
    osc_time_window = min(time_window, cycles_to_show / frequency)
    mask = t <= osc_time_window
    
    if device in ["heater", "motor"]:
        ax2 = ax.twinx()
        ax2.plot(t[mask], output[mask], linewidth=2.5, color="lime", label=f"{device.capitalize()} Output")
        ax2.set_ylabel("Temperature (°C)" if device == "heater" else "Speed (RPM)", color="lime")
        ax2.tick_params(axis='y', colors="lime")
        ax.plot(t[mask], pwm[mask], linestyle="solid", linewidth=2, color="cyan", label="PWM Input (V)")
        ax.set_ylabel("PWM Logic Level (V)", color="cyan")
    else:
        ax.plot(t[mask], output[mask], linewidth=2.5, color="lime", label=f"{device.capitalize()} Output")
        ax.plot(t[mask], pwm[mask], linestyle="solid", linewidth=2, color="cyan", label="PWM Input (V)")
        ax.set_ylabel("Amplitude (V)", color="white")

    ax.set_title(f"Oscilloscope View - Zoomed to {cycles_to_show} Cycles", color="white")
