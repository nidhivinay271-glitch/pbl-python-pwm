...
# =============================================================================
# ADVANCED FEATURES (FULLY RESTORED & UPGRADED LIST)
# =============================================================================
st.markdown("---")
...
if advanced_feature == "FFT Analyzer":
    ...
    st.info("Calculates true logic conversion efficiency. High PWM frequencies can introduce switching losses not captured here.")
    ...
    st.dataframe(df.head(100), use_container_width=True) # Show first 100 rows to prevent browser lag
    st.caption("Showing the first 100 sampled points. Download the CSV above for full data.")
    ...
    if duty_cycle > 90 and device in ["motor", "heater", "transistor", "led"]:
        ...
        st.markdown("Ensure proper heat sinking. Running near 100% continuous duty defeats the purpose of PWM efficiency.")
    elif frequency > 15000 and device in ["transistor", "motor"]:
        st.warning(f"CAUTION: {frequency}Hz switching frequency induces high dynamic switching losses. Transistor gates may struggle to charge fully without a dedicated gate driver.")
    else:
